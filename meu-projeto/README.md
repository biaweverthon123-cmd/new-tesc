...
3. Execute `npm run prisma:migrate -- --name configurar_backend`.
4. Inicie com `npm run dev`.

O backend usa `http://localhost:3001` e aceita o frontend em `http://localhost:3000`.

Para a busca automática do brasão, configure `GOOGLE_CSE_API_KEY` e `GOOGLE_CSE_CX` no `.env`. O mecanismo de busca programável precisa permitir pesquisa de imagens.
